...
app.get('/api/items', authenticateApiKey, async (req, res) => {
  try {
    // Check if we should include completed/canceled items (default: false)
    ...
    // Filter out Completed and Canceled by default (Issue #2)
    if (!includeCompleted) {
...
app.post('/api/items', authenticateApiKey, async (req, res) => {
  try {
    ...
    // Remove sequential_id if present - it's auto-generated by the database
    delete (itemData as any).sequential_id;

    // Default to 'Open' status if not provided (Issue #2)
    if (!itemData.status) {
...
app.put('/api/items/:id', authenticateApiKey, async (req, res) => {
  try {
    ...
    delete itemData.sequential_id; // Don't update sequential_id (Issue #3)
